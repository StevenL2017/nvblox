...
class Mapper : public MapperBase {
 public:
  ...
  /// Whether to collect observed TSDF blocks within the current field of view
  /// and mark them as newly added.
  bool add_observed_blocks_in_fov() const {
    return add_observed_blocks_in_fov_;
  }
  /// Setter for add_observed_blocks_in_fov().
  void add_observed_blocks_in_fov(bool add_observed_blocks_in_fov) {
    add_observed_blocks_in_fov_ = add_observed_blocks_in_fov;
  }
 ...
 protected:
  ...
  bool add_observed_blocks_in_fov_ =
      kAddObservedBlocksInFovParamDesc.default_value;
